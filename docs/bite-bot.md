...
| `MIN_BURN_USD` | no | default `50` — minimum USD for Telegram burn posts (not points). |
| `MIN_BURN_SCORE_USD` | no | default `0` — every kitchen burn scores at `BURN_SCORE_MULT`. |
| `BUY_SCORE_MULT` | no | default `0.01` — points per $BITE bought |
| `SELL_SCORE_MULT` | no | default `0.015` — 1.5× buy |
| `BURN_SCORE_MULT` | no | default `1` — points per $BITE burned (bigger bite than trades) |
| `WAGER_SCORE_MULT` | no | default `0.001` — points per $BITE staked on MetaWager entry (gross = net + 10% fee). Side bet, not a bite. |
| `EARLY_EATER_BURN_MULT` | no | default `2` — extra burn multiplier in the first 72h of Act II |
| `ACT_II_STARTED_AT` | no | unix seconds; required for the early-eater 2× window on the bot |
| `PHASE` | no | `1` Act I; `2+` enable burn posts |
| `POINTS_PER_BITE_GAINED` | no | default `0.01` — Act I accum |
| `HOLD_BITE_PER_POINT_PER_HOUR` | no | default `10000` — 100 $BITE held 1 hour = 0.01 pt |
...
| `PONS_FEE_ESCROW` | no | default `0xd3AF…Ac9e` — Pons V2 fee escrow for kitchen-attributed claimable AAPL |
| `KITCHEN_CONTRACT` | no | default `0x56fE…81e0` — Apple kitchen |
...
| `/stats` | `/supply`, `/info`, `stats`, `check stats`, `supply` | Supply breakdown: total, burned %, EOA held, LP/contract held, realistically burnable, prize pool (kitchen AAPL + Pons escrow claimable for the kitchen, not auto-claimed), holders, price |
...
- Data source: `BITE_LEADERBOARD_URL` (optional, 4s timeout) → `public/data/act1-leaderboard.json` → `BITE_BOT_STATE_FILE`.
- Site overlays a **live prize pool** on every `/api/leaderboard` and `/api/race` request: kitchen AAPL balance + `PonsV2FeeEscrow.balanceOfToken(kitchen, AAPL)` (display only, never claimed). Totals/price also refresh from RPC + Dexscreener so Vercel is not stuck on a baked JSON snapshot.
- Home and `/leaderboard` poll that payload every **15s**.
...
- **Railway** (recommended): see [Railway deployment](#9-railway-deployment) below.
- **Fly.io / Render**: deploy a worker that runs `python -m bots --daemon`; set env vars in the platform dashboard (same names as `bots/.env.example`).
...
Railway provides persistent hosting with volumes for state files and automatic restarts. The repo includes `bots/Dockerfile` and `railway.toml` ready to go.
...
# 3. Add a persistent volume (keeps state across deploys)
railway volume add --mount /data

# 4. Set environment variables (see table below — never commit secrets)
railway variables set TELEGRAM_BOT_TOKEN="your-token-here"
railway variables set TELEGRAM_CHAT_ID="your-chat-id"
railway variables set BLOCKSCOUT_API_KEY="your-key"
# ... set all vars from the table below

# 5. Deploy
railway up
```

Or deploy from the **Railway dashboard**:

1. New Project → Deploy from GitHub Repo → select this repo
2. Railway auto-detects `railway.toml` + `bots/Dockerfile`
3. Add a **Volume** mounted at `/data` (Settings → Volumes → Add Volume → Mount Path: `/data`)
4. Add environment variables in Settings → Variables (see table below)
5. Deploy

### Environment variables for Railway

**Secrets** (set in Railway dashboard, never commit):
...
| `TWITTER_API_KEY` | Optional — skip if no Twitter |
| `TWITTER_API_SECRET` | Optional |
| `TWITTER_ACCESS_TOKEN` | Optional |
| `TWITTER_ACCESS_SECRET` | Optional |

**Config** (set in Railway dashboard or `railway.toml`):

| Variable | Value | Notes |
|----------|-------|-------|
| `BITE_CONTRACT` | `0x0d6e3D5D99a92499f584Ac821a64b237e5cEf3c9` | Default; only change for testnet |
| `RPC_URL` | `https://rpc.mainnet.chain.robinhood.com` | Default |
| `CHAIN_ID` | `4663` | Default |
| `PHASE` | `2` | `1` = Act I, `2+` = burn posts enabled |
| `POST_ACTIVITY` | `1` | `1` to post buy alerts |
| `TG_POST_COOLDOWN` | `60` | Seconds between broadcast posts |
| `SITE_URL` | `https://www.bite.party` | Default |
| `POINTS_PER_BITE_GAINED` | `0.01` | Default |
| `HOLD_BITE_PER_POINT_PER_HOUR` | `10000` | Default |
| `LEADERBOARD_TOP_N` | `10` | Default |
| `DEV_WALLETS` | `0xEB95ff72EAb9e8D8fdb545FE15587AcCF410b42E` | Default |
| `BLOCKSCOUT_API_BASE` | `https://api.blockscout.com/4663/api/v2` | Default |
| `DEXSCREENER_PAIR_ID` | `0x76d38162...` | Default |
| `TRADE_SCAN_FROM_BLOCK` | `63818043` | Default |
| `LOG_CHUNK_SIZE` | `10000` | Default |

**Auto-set by Dockerfile** (don't override unless needed):

| Variable | Value | Notes |
|----------|-------|-------|
| `BITE_BOT_STATE_FILE` | `/data/state.json` | Volume-backed; survives redeploys |
| `BITE_LEADERBOARD_PUBLIC_FILE` | `/data/act1-leaderboard.json` | Written by bot for HTTP serving |

### State persistence

The Dockerfile sets `BITE_BOT_STATE_FILE=/data/state.json`. Railway volumes persist across deploys, so:

- Points, wallet links, trade index, and milestones survive redeploys
- If you delete the volume, the bot re-backfills from `TRADE_SCAN_FROM_BLOCK` on next start (safe but slower first boot)
- Hold/accum points from the gap period are seeded from Blockscout on restart

### Leaderboard sync (bot → Vercel site)

The bot can't write directly to Vercel's filesystem. Instead:

1. **Bot serves leaderboard via HTTP** — when Railway assigns a `PORT` (or you set `LEADERBOARD_HTTP_PORT`), the daemon starts a lightweight HTTP server serving:
   - `GET /leaderboard.json` — current Act I leaderboard payload
   - `GET /health` — health check

2. **Vercel fetches from Railway** — on the Vercel project, set:
   ```
   BITE_LEADERBOARD_URL=https://your-railway-service.up.railway.app/leaderboard.json
   ```
   The site's `fetchAct1Leaderboard()` in `src/lib/act1-leaderboard.ts` already supports this: it tries `BITE_LEADERBOARD_URL` first, then falls back to the local public JSON, then bot state.

3. **Railway networking** — to get a public URL for the leaderboard endpoint:
   - Railway dashboard → your service → Settings → Networking → Generate Domain
   - Or: Settings → Networking → Custom Domain
   - The generated URL (e.g. `bite-bot-production.up.railway.app`) serves the leaderboard JSON

4. **Set on Vercel** — in the Vercel project settings or `.env.production`:
   ```
   BITE_LEADERBOARD_URL=https://bite-bot-production.up.railway.app/leaderboard.json
   ```

### Monitoring

- **Logs**: `railway logs` or Railway dashboard → Deployments → View Logs
- **Health**: Railway uses `/health` endpoint for health checks (configured in `railway.toml`)
- **Restart policy**: auto-restarts on failure (up to 10 retries, configured in `railway.toml`)

### Troubleshooting

| Issue | Fix |
|-------|-----|
| RPC timeouts | Bot retries on next poll cycle (30s default). Railway's stable networking helps vs local ISP drops. |
| Bot not posting | Check logs for `[TG error]`. Verify `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` are set. |
| Leaderboard empty on site | Verify `BITE_LEADERBOARD_URL` is set on Vercel and the Railway URL is reachable. |
| State lost on redeploy | Ensure a Railway volume is mounted at `/data`. Check with `railway volume list`. |
| Duplicate posts after restart | Bot seeds milestone markers on startup — skips already-passed milestones. Catch-up mode suppresses TG posts when >10K blocks behind. |
